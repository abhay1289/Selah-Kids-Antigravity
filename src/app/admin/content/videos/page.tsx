                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="overflow-hidden">
                    <div className="p-6 border-t border-[#3a6b44]/5 space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div><label className="text-[12px] font-semibold text-[#3a6b44] mb-1.5 block">Category</label><select value={CATEGORIES.includes(video.category) ? video.category : 'music-video'} onChange={(e) => updateVideo(video.id, 'category', e.target.value as EpisodeCategory)} className="w-full h-[44px] px-4 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 text-[#3a6b44] text-[14px] font-medium outline-none">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
                        <div><label className="text-[12px] font-semibold text-[#3a6b44] mb-1.5 block">Language</label><select value={LANGUAGES.includes(video.language) ? video.language : 'EN'} onChange={(e) => updateVideo(video.id, 'language', e.target.value as EpisodeLanguage)} className="w-full h-[44px] px-4 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 text-[#3a6b44] text-[14px] font-medium outline-none">{LANGUAGES.map((l) => <option key={l} value={l}>{l}</option>)}</select></div>
                        <div><label className="text-[12px] font-semibold text-[#3a6b44] mb-1.5 block">Age Band</label><select value={AGE_BANDS.includes(video.ageBand) ? video.ageBand : 'all'} onChange={(e) => updateVideo(video.id, 'ageBand', e.target.value as AgeBand)} className="w-full h-[44px] px-4 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 text-[#3a6b44] text-[14px] font-medium outline-none">{AGE_BANDS.map((a) => <option key={a} value={a}>{a}</option>)}</select></div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className="text-[12px] font-semibold text-[#3a6b44] mb-1.5 block">Duration (seconds)</label><input type="number" value={typeof video.durationSec === 'number' ? video.durationSec : 0} onChange={(e) => updateVideo(video.id, 'durationSec', Math.max(0, parseInt(e.target.value, 10) || 0))} className="w-full h-[44px] px-4 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 focus:bg-white text-[#3a6b44] text-[14px] font-medium outline-none transition-all" /></div>
                      </div>
                      ...
                      <div><label className="text-[12px] font-semibold text-[#3a6b44] mb-1.5 block">Tags (comma-separated)</label><input value={(Array.isArray(video.tags) ? video.tags : []).join(', ')} onChange={(e) => updateVideo(video.id, 'tags', e.target.value.split(',').map(t => t.trim()).filter(Boolean))} placeholder="joy, praise, worship" className="w-full h-[44px] px-4 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 focus:bg-white text-[#3a6b44] text-[13px] font-medium outline-none transition-all" /></div>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-[#3a6b44]/5">
                        <div className="flex items-center gap-3">
                          ...
                          <label className="flex items-center gap-2 text-[12px] font-semibold text-[#3a6b44] cursor-pointer"><input type="checkbox" checked={video.sensoryFriendly === true} onChange={(e) => updateVideo(video.id, 'sensoryFriendly', e.target.checked)} /> Sensory-friendly</label>
                        </div>
