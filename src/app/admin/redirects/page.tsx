...
export default function RedirectsManager() {
  ...
  const update = (id: string, field: keyof Redirect, value: any) => setRedirects(redirects.map(r => r.id === id ? { ...r, [field]: value } : r));
          ...
          <motion.div key={redirect.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-sm p-5">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-3">
              ...
              <div className="w-[100px]">
                ...
                <select value={redirect.type} onChange={e => update(redirect.id, 'type', e.target.value)} className="w-full h-[40px] px-3 rounded-xl bg-[#3a6b44]/[0.02] border-2 border-transparent focus:border-[#ff5c00]/20 text-[#3a6b44] text-[13px] font-bold outline-none transition-all appearance-none cursor-pointer">
