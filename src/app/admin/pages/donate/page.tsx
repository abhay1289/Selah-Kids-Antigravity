...
} from '../../../../data/page-content-donate';

export default function DonatePageEditor() {
  const fallback = useMemo(buildDonateFallback, []);
  const { fields, setField, isSaving, save, error } = useCmsPageContent('donate', fallback);
  const [editedKeys, setEditedKeys] = useState<Set<string>>(new Set());
  const getVal = (f: PageEditorField, lang: 'en' | 'es') => {
    const v = fields[donateKeyFor(f.id)];
    return v ? v[lang] : (lang === 'en' ? f.valueEn : f.valueEs);
  };
  const setVal = (fid: string, lang: 'en' | 'es', v: string) => {
    const k = donateKeyFor(fid);
