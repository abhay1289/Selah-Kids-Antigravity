...
} from '../../../../data/page-content-blog';

/**
 * Blog list page chrome editor — hero + category labels + empty-state copy.
 * Per-post content lives under the `blog_posts` collection; this editor
 * only covers the strings that frame the list page around those posts.
 */
export default function BlogPageEditor() {
  const fallback = useMemo(buildBlogFallback, []);
  const { fields, setField, isSaving, save, error } = useCmsPageContent('blog', fallback);
  const [editedKeys, setEditedKeys] = useState<Set<string>>(new Set());
  const getVal = (f: PageEditorField, lang: 'en' | 'es') => {
    const v = fields[blogKeyFor(f.id)];
    return v ? v[lang] : (lang === 'en' ? f.valueEn : f.valueEs);
  };
  const setVal = (fid: string, lang: 'en' | 'es', v: string) => {
    const k = blogKeyFor(fid);
    ...
    <div className="max-w-[900px] mx-auto space-y-6">
      <div className="flex items-center justify-between bg-white/80 backdrop-blur-xl rounded-2xl px-6 py-4 border border-white/60 shadow-sm sticky top-[72px] z-20">
        <div className="flex items-center gap-3">
          <h2 className="text-[16px] font-bold text-[#3a6b44]" style={{ fontFamily: 'var(--font-fredoka)' }}>Blog Page</h2>
