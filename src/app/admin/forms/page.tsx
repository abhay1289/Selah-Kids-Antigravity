...
export default function FormsManager() {
  ...
  const updateForm = (field: keyof FormConfig, value: any) => setForms(forms.map(f => f.id === selected ? { ...f, [field]: value } : f));
  const updateField = (fieldId: string, key: keyof FormField, value: any) => { if (!form) return; const fields = form.fields.map(f => f.id === fieldId ? { ...f, [key]: value } : f); updateForm('fields', fields); };
              ...
              {form.fields.map((field, i) => (
                <div key={field.id} className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-sm p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <select value={field.type} onChange={e => updateField(field.id, 'type', e.target.value)} className="h-[28px] px-2 rounded-lg bg-[#3a6b44]/[0.03] text-[11px] font-bold text-[#3a6b44] outline-none appearance-none cursor-pointer"><option value="text">Text</option><option value="email">Email</option><option value="phone">Phone</option><option value="textarea">Textarea</option><option value="select">Select</option></select>
