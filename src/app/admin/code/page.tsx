...
export default function CustomCode() {
  ...
  const update = (id: string, field: keyof CodeBlock, value: any) => setBlocks(blocks.map(b => b.id === id ? { ...b, [field]: value } : b));
        ...
        <motion.div key={block.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-sm overflow-hidden">
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div><label className="text-[10px] font-bold text-[#5a7d62]/30 uppercase mb-0.5 block">Placement</label><select value={block.placement} onChange={e => update(block.id, 'placement', e.target.value)} className="h-[32px] px-3 rounded-lg bg-[#3a6b44]/[0.03] text-[#3a6b44] text-[12px] font-medium outline-none appearance-none cursor-pointer"><option value="head">{PLACEMENT_LABELS['head']}</option><option value="body-start">{PLACEMENT_LABELS['body-start']}</option><option value="body-end">{PLACEMENT_LABELS['body-end']}</option></select></div>
