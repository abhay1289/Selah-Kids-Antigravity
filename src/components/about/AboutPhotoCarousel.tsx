  ...
  <AnimatePresence>
    {src && (
      <motion.div
        ...
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/85 backdrop-blur-xl cursor-pointer"
        onClick={onClose}
      >
        {/* Close button */}
        <motion.button
          ...
          className="absolute top-6 right-6 p-3 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-white transition-colors backdrop-blur-sm cursor-pointer z-10"
          onClick={onClose}
        ...
        <motion.div
          ...
          className="relative w-[90vw] h-[80vh] max-w-[1200px] rounded-3xl overflow-hidden shadow-2xl"
        ...
        <motion.p
          ...
          className="absolute bottom-6 text-white/50 text-sm font-sans"
        >
...
export const AboutPhotoCarousel = () => {
  ...
  return (
    <>
      <section
        ref={sectionRef}
        className="py-12 md:py-16 bg-selah-bg relative overflow-hidden"
        id="photo-carousel"
      >
        ...
        >
          <motion.span
            ...
            viewport={{ once: true }}
            className="ui-label text-selah-orange inline-block mb-4 tracking-[0.2em]"
          >
            Gallery
          </motion.span>
          <motion.h2
          ...
          <motion.p
            ...
            className="body-text mx-auto text-center"
        ...
        {/* Row 1 — continuous scroll left + scroll-reactive parallax */}
        <div className="relative mb-6 md:mb-8">
          ...
          >
            {row1Items.map((src, i) => (
              <PhotoCard key={`r1-${i}`} src={src} index={i} onOpen={openLightbox} />
            ))}
          </motion.div>
        </div>

        {/* Row 2 — continuous scroll right + scroll-reactive parallax */}
        <div className="relative">
          ...
          >
            {row2Items.map((src, i) => (
              <PhotoCard key={`r2-${i}`} src={src} index={i} onOpen={openLightbox} />
            ))}
          </motion.div>
        </div>
      </section>

      {/* Lightbox (portalled to top-level) */}
