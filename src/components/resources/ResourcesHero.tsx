...
export const ResourcesHero = ({ fields }: { fields?: PageFieldMap } = {}) => {
  const f = useFieldResolver(fields);

  const fullTitle = f('general.title', 'Resources for Families', 'Recursos para Familias');
  // Split the resolved title into leading words + last word (emphasised).
  // Preserves the original word-by-word animation regardless of whether the
  // admin-edited copy is 2, 3, or more words long.
  const titleWords = fullTitle.trim().split(/\s+/);
  const lastWord = titleWords.length > 0 ? titleWords[titleWords.length - 1] : '';
  const leadingWords = titleWords.slice(0, -1);
  ...
  return (
    <section className="text-center px-6 mb-16 relative z-10 max-w-5xl mx-auto">
      ...
      >
        ...
        <span className="ui-label text-selah-dark/90 relative z-10">{f('general.badge', 'FREE RESOURCES FOR FAMILIES', 'RECURSOS GRATUITOS PARA FAMILIAS')}</span>
      </motion.div>
